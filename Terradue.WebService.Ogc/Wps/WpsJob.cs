...
namespace Terradue.WebService.Ogc.Wps
{
    public class WpsJob : IJob<ExecuteResponse>
    {
        ...
        readonly WpsProcess wpsProcess;

        private static readonly ILog log = LogManager.GetLogger
            (System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);

        static MemoryCache wpsJobCache = new MemoryCache("wpsJobs");
        ...
        private string Save(DateTimeOffset absoluteExpiration)
        {
            ...
            while (wpsJobCache.Contains(uid.ToString()))
            {
                Thread.Sleep(100);
                uid = Guid.NewGuid();
            }
            lock (wpsJobCache)
            {
                wpsJobCache.Add(uid.ToString(), this, absoluteExpiration.DateTime);
            }
            return uid.ToString();
        }

        private static WpsJob Load(string uid)
        {
            WpsJob job = null;
            lock (wpsJobCache)
            {
                job = wpsJobCache.Get(uid) as WpsJob;
            }
            if (job == null)
                return null;
            return job;
        ...
        public static ExecuteResponse GetCachedExecuteResponse(string uid)
        {
            var job = WpsJob.Load(uid);
            if (job == null)
            {
